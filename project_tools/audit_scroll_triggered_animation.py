...
.trigger-visual{width:160px;height:120px;box-sizing:border-box;padding:18px;color:white;background:rgb(220,0,0);transform:translateX(0px);animation:audit-reveal 220ms linear both;animation-trigger:--audit-trigger play-forwards reset}
@keyframes audit-reveal{from{background:rgb(220,0,0);transform:translateX(0px)}to{background:rgb(0,150,0);transform:translateX(260px)}}
</style>"""
...
def support_state(page):
    return page.evaluate("""()=>({timelineTrigger:CSS.supports('timeline-trigger: --audit-trigger view() contain / cover'),animationTrigger:CSS.supports('animation-trigger: --audit-trigger play-forwards reset'),animationTriggerBasic:CSS.supports('animation-trigger: --audit-trigger play-forwards play-backwards')})""")
