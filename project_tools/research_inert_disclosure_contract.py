...
<script>
window.research={clicks:[],submits:0,toggles:[],mutations:[],statefulCreated:0,nativeToggleCreated:0};
document.addEventListener('click',e=>{if(['aria','stateful','submitCtrl','anchorCtrl','outsideCtrl'].includes(e.target.id)){research.clicks.push(e.target.id);if(e.target.id==='anchorCtrl')e.preventDefault()}},true);
document.querySelector('#form').addEventListener('submit',e=>{research.submits++;e.preventDefault()});
for(const d of document.querySelectorAll('details'))d.addEventListener('toggle',()=>research.toggles.push(d.id));
document.querySelector('#nativeClosed').addEventListener('toggle',()=>{if(document.querySelector('#nativeClosed').open&&!document.querySelector('#nativeToggleCreated')){const x=document.createElement('p');x.id='nativeToggleCreated';x.textContent='NATIVE_TOGGLE_CREATED_CONTENT';document.querySelector('#nativeClosed').appendChild(x);research.nativeToggleCreated++}});
document.querySelector('#stateful').addEventListener('click',()=>{const p=document.querySelector('#statefulPanel');p.hidden=false;document.querySelector('#stateful').setAttribute('aria-expanded','true');if(!document.querySelector('#created')){const x=document.createElement('div');x.id='created';x.textContent='STATEFUL_CREATED_BY_SYNTHETIC_CLICK';p.appendChild(x);research.statefulCreated++}});
new MutationObserver(ms=>{research.mutations.push(...ms.map(m=>({type:m.type,target:m.target.id||m.target.tagName,attr:m.attributeName||''})));}).observe(document.querySelector('#scope'),{subtree:true,attributes:true,childList:true});
window.researchState=()=>({research:JSON.parse(JSON.stringify(research)),closedOpen:document.querySelector('#nativeClosed').open,nestedOpen:document.querySelector('#nested').open,openOpen:document.querySelector('#nativeOpen').open,ariaExpanded:document.querySelector('#aria').getAttribute('aria-expanded'),statefulExpanded:document.querySelector('#stateful').getAttribute('aria-expanded'),submitExpanded:document.querySelector('#submitCtrl').getAttribute('aria-expanded'),anchorExpanded:document.querySelector('#anchorCtrl').getAttribute('aria-expanded'),outsideExpanded:document.querySelector('#outsideCtrl').getAttribute('aria-expanded'),created:!!document.querySelector('#created')});
</script>'''
...
async def main_run(chromium,out):
  async with async_playwright() as p:
    ...
    before=await page.evaluate('researchState()'); t0=time.perf_counter(); model=await page.evaluate('webclipModel.expandSpoilersInIncludedContent()'); elapsed_ms=(time.perf_counter()-t0)*1000; await page.wait_for_timeout(50); prepared=await page.evaluate('researchState()')
    await page.add_style_tag(content='[data-webclip-pdf-exclude],[data-webclip-pdf-exclude] *{display:none!important}')
    top_pdf=await make_pdf(page,out,'top_disclosures'); after_pdf=await page.evaluate('researchState()')
    ...
    assert prepared['research']['clicks'].count('aria')==1 and prepared['ariaExpanded']=='true'
    assert prepared['research']['clicks'].count('stateful')==1 and prepared['created'] is True and 'STATEFUL_CREATED_BY_SYNTHETIC_CLICK' in top_pdf['text']
    assert prepared['research']['submits']==1 and prepared['research']['clicks'].count('submitCtrl')==1
    assert prepared['research']['clicks'].count('anchorCtrl')==1
    assert prepared['research']['clicks'].count('outsideCtrl')==0 and prepared['outsideExpanded']=='false' and 'OUTSIDE_PANEL' not in top_pdf['text']
