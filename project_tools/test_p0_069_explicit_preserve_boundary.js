...
throwsCode(
  () => context.resolveForTest(published, 'revoke'),
  'WEBCLIP_PUBLICATION_REVOKE_UNAVAILABLE',
  'unimplemented revoke must fail before destructive work'
);
...
const resolveCall = deleteSource.indexOf('resolveJournalDeletePublicationOutcome(entry, publicationAction)');
...
ok(page.includes('const publicationChoiceReady = !pendingDeleteHasKnownPublicAccess() || deletePreservePublicAccess.checked;'), 'proceed remains disabled until explicit preserve');
ok(page.includes("? (deletePreservePublicAccess.checked ? 'preserve' : '')"), 'page derives preserve only from deliberate selection');
...
ok(!deleteSource.includes("'/resources/unpublish'"), 'this bounded tranche must not pretend revoke was implemented');
