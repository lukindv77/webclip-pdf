...
`const waitComplete=async id=>{for(let i=0;i<150;i++){const t=await chrome.tabs.get(id);if(t.status==='complete')return;await sleep(100)}throw new Error('fixture-load-timeout')};\n` +
`const attached=async id=>Boolean((await chrome.debugger.getTargets()).find(x=>x.tabId===id)?.attached);\n` +
`const send=async(d,m,p)=>chrome.debugger.sendCommand(d,m,p);\n` +
`const run=async(name,params)=>{const r={name,events:[],steps:[]};let tab,dbg;const detachListener=(source,reason)=>{if(tab&&source?.tabId===tab.id)r.events.push({event:'onDetach',reason:String(reason||'')})};chrome.debugger.onDetach.addListener(detachListener);try{tab=await chrome.tabs.create({url:${JSON.stringify(fixture)},active:false});dbg={tabId:tab.id};await waitComplete(tab.id);r.attachedBefore=await attached(tab.id);await chrome.debugger.attach(dbg,'1.3');r.attachedAfterAttach=await attached(tab.id);await send(dbg,'Page.enable');await send(dbg,'Runtime.enable');await send(dbg,'Emulation.setEmulatedMedia',{media:'screen'});try{r.printResult=await send(dbg,'Page.printToPDF',params);r.printOk=true}catch(e){r.printOk=false;r.printError=String(e?.message||e)}r.attachedImmediatelyAfterPrint=await attached(tab.id);for(const [label,method,p] of [['Page.getLayoutMetrics','Page.getLayoutMetrics',undefined],['Runtime.evaluate','Runtime.evaluate',{expression:'6*7',returnByValue:true}],['Page.captureScreenshot','Page.captureScreenshot',{format:'png'}]]){try{const v=await send(dbg,method,p);r.steps.push({label,ok:true,summary:label==='Runtime.evaluate'?v?.result?.value:label==='Page.captureScreenshot'?String(v?.data||'').length:'ok'})}catch(e){r.steps.push({label,ok:false,error:String(e?.message||e)})}}r.attachedAfterFollowups=await attached(tab.id);await sleep(250);r.eventsBeforeExplicitDetach=[...r.events];try{await chrome.debugger.detach(dbg);r.explicitDetachOk=true}catch(e){r.explicitDetachOk=false;r.explicitDetachError=String(e?.message||e)}await sleep(250);r.attachedAfterExplicitDetach=await attached(tab.id);r.eventsAfterExplicitDetach=[...r.events]}catch(e){r.topError=String(e?.stack||e)}finally{chrome.debugger.onDetach.removeListener(detachListener);try{if(tab?.id)await chrome.tabs.remove(tab.id)}catch(_){}}out.tests.push(r)};\n` +
...
`const data=encodeURIComponent(JSON.stringify(out));await chrome.tabs.create({url:${JSON.stringify(report)}+'?data='+data,active:false});\n` +
...
try {
  ...
  const result = await Promise.race([fixture.reportPromise,new Promise((_,reject)=>setTimeout(()=>reject(new Error('C46 lifecycle report timeout')),60000))]);
