...
def assert_source_bound() -> dict[str, str]:
    ...
    if "for (const node of [...sourceBody.childNodes])" not in content:
        raise AssertionError("content.js no longer exposes the researched childNodes preflight boundary")
    if "proxy.appendChild(node.cloneNode(true))" not in content:
        raise AssertionError("content.js no longer exposes the researched deep clone boundary")
    if "[sourceBody, ...sourceBody.querySelectorAll('*')]" not in content:
        raise AssertionError("content.js no longer exposes the researched full source array boundary")
