...
() => {
  ...
  globalThis.__makeRegistry = (color, visibleText) => {
    ...
    registry.define('research-card', class extends HTMLElement {
      connectedCallback() {
  ...
  globalThis.__makeScopedHost = (registry, marker) => {
    ...
    host.innerHTML = `<research-card></research-card><span>${marker}</span>`;
...
def scoped_hosts(page):
    return page.evaluate(
        """() => [...document.querySelectorAll('.scoped-host')].map((e,i) => ({
          ...
          light:e.textContent||'', visual:e.querySelector('research-card')?.shadowRoot?.textContent||''
        }))"""
    )
    ...
    with sync_playwright() as pw:
        ...
        support = page.evaluate(
            """() => {
              ...
              return {
                ...
                aVisual:ea.querySelector('research-card')?.shadowRoot?.textContent||'',
                bVisual:eb.querySelector('research-card')?.shadowRoot?.textContent||'',
        ...
        page.evaluate(
            """() => {
              ...
              s.innerHTML='<research-card></research-card>';
            }"""
        )
        start(page)
        ...
        page.evaluate(
            """strong => {
              ...
              r.define('research-main',class extends HTMLElement{
                connectedCallback(){
                  if(this.shadowRoot)return;
                  const s=this.attachShadow({mode:'open'});
              ...
              const s=h.attachShadow({mode:'open',customElementRegistry:r});
              s.innerHTML='<research-main></research-main>';
            }""",
            strong,
        )
        composed = page.evaluate(
            """() => ({
              ...
              shadowArticle:!!document.querySelector('#xroot').shadowRoot
                .querySelector('research-main').shadowRoot.querySelector('article[role=main]'),
        ...
        before = page.evaluate(
            """() => {
              ...
              r.define('research-card',class extends w.HTMLElement{
                connectedCallback(){
                  if(this.shadowRoot)return;
                  const s=this.attachShadow({mode:'open'});
              ...
              const s=h.attachShadow({mode:'open',customElementRegistry:r});
              s.innerHTML='<research-card></research-card>';
              const card=s.querySelector('research-card');
              return {
