...
def physical_case(ctx,out_dir,name,html,include,exclude=""):
    ...
    assert command(page,{"type":"WEBCLIP_COMMAND","command":"start"}).get("ok") is True; click(page,include)
    if exclude: assert command(page,{"type":"WEBCLIP_COMMAND","command":"mode-exclude"}).get("ok") is True; click(page,exclude)
    assert command(page,{"type":"WEBCLIP_COMMAND","command":"download"}).get("ok") is True; modal_click(page); request=wait_prepared(page)
    assert request.get("meta",{}).get("selectionSnapshot",{}).get("includes"),request
...
def run(chromium,out_dir):
    ...
    with sync_playwright() as pw:
        ...
        basic="""<div>C04_OUTSIDE_NOISE</div><article id='basic' class='scope'><h1>C04_BASIC_HEADING</h1><p>C04_PARA_A <strong>C04_STRONG</strong> <em>C04_EMPHASIS</em> <span>C04_INLINE</span></p><section><p>C04_NESTED_PARA</p></section><p>C04_PARA_B</p></article><footer>C04_OUTSIDE_FOOTER</footer>"""
        a=physical_case(ctx,out_dir,"basic_nested",basic,"#basic"); markers=["C04_BASIC_HEADING","C04_PARA_A","C04_STRONG","C04_EMPHASIS","C04_INLINE","C04_NESTED_PARA","C04_PARA_B"]; assert_in_order(a["text"],markers); assert "C04_OUTSIDE_NOISE" not in a["text"] and "C04_OUTSIDE_FOOTER" not in a["text"]; results["basicNested"]={"bytes":a["bytes"],"sha256":a["sha256"],"markers":markers,"pageAnalysis":a["pageAnalysis"]}
        ...
        a=physical_case(ctx,out_dir,"nested_exclude",ex,"#outer","#cut"); assert_in_order(a["text"],["C04_KEEP_BEFORE","C04_KEEP_AFTER"]); assert "C04_EXCLUDE_PARENT" not in a["text"] and "C04_EXCLUDE_NESTED" not in a["text"] and "C04_EXCLUDE_OUTSIDE_NOISE" not in a["text"]; results["nestedExclude"]={"bytes":a["bytes"],"sha256":a["sha256"],"pageAnalysis":a["pageAnalysis"]}
        ...
        a=physical_case(ctx,out_dir,"breaks_pre",br,"#breaks"); assert_in_order(a["text"],["C04_BR_ALPHA","C04_BR_BETA","C04_BR_GAMMA","C04_PRE_ALPHA","C04_PRE_BETA","C04_PRE_GAMMA","C04_ENTITY_AMP","&","C04_ENTITY_LITERAL"]); results["breaksPre"]={"bytes":a["bytes"],"sha256":a["sha256"],"rawTextLines":[x for x in a["text"].splitlines() if x.strip()][:24],"pageAnalysis":a["pageAnalysis"]}
        ...
        a=physical_case(ctx,out_dir,"semantic_text",sem,"#semantic"); markers=["C04_SECTION_HEADING","C04_UL_ONE","C04_UL_TWO","C04_OL_ONE","C04_OL_TWO","C04_TH_A","C04_TH_B","C04_R1_A","C04_R1_B","C04_R2_A","C04_R2_B","C04_SECTION_END"]; assert_in_order(a["text"],markers); results["semanticText"]={"bytes":a["bytes"],"sha256":a["sha256"],"markers":markers,"pageAnalysis":a["pageAnalysis"]}
