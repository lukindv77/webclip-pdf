...
def assert_current_source() -> dict[str, str]:
    ...
    if content.count(".cloneNode(true)") != 1 or "proxy.appendChild(node.cloneNode(true))" not in content:
        raise AssertionError("content.js deep-clone interception boundary changed; re-research required")
    return {
...
def native_clone_baseline(page: Page, state: FixtureState) -> dict[str, Any]:
    ...
    result = page.evaluate("""() => {
      ...
      clone.setAttribute('data-research-proxy', 'baseline');
    ...
    page.evaluate("document.querySelector('[data-research-proxy=baseline]')?.remove()")
...
def guarded_clone(worker, tab_id: int) -> dict[str, Any]:
    results = worker.evaluate("""async (tabId) => chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        const source = document.getElementById('sourceFrame').contentDocument.getElementById('selected');
        const clone = source.cloneNode(true);
        clone.setAttribute('data-research-proxy', 'guarded');
...
def main() -> int:
    ...
    try:
        with sync_playwright() as p:
            ...
            after_requests = state.snapshot()
            dom = page.evaluate("""() => {
              const clone = document.querySelector('[data-research-proxy=guarded]');
            ...
            page.evaluate("document.querySelector('[data-research-proxy=guarded]')?.remove()")
