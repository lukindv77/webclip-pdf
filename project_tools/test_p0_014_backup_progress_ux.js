...
const researchRegistry = read('project_docs/RESEARCH_REGISTRY.md');
...
assert(/## Default status for legacy assigned P0\/P1 codes[\s\S]*?\*\*IMPLEMENTED \/ RELEASE-REGRESSION\*\*/.test(researchRegistry), 'canonical research registry must preserve the default implemented/release-regression state for legacy assigned P0/P1 codes');
assert(!/^\|\s*P0-014\s*\|\s*(?:ACTIVE|DONE|MERGED|SUPERSEDED)/m.test(researchRegistry), 'P0-014 must not be overridden to a different canonical status without updating this regression gate');
