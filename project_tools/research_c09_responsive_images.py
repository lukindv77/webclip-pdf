...
def run(browser, base: str, out: pathlib.Path) -> dict[str, object]:
    ...
    p.evaluate("""helper=>{const f=document.querySelector('#f'),s=f.contentDocument.querySelector('#p'),t=s.cloneNode(true);eval(helper)(s.querySelector('img'),t.querySelector('img'));document.querySelector('#m').appendChild(t);f.remove()}""",HELPER); immediate=p.eval_on_selector('#m img','i=>({src:i.currentSrc,complete:i.complete,nw:i.naturalWidth})'); first=pdf(p,out,'delayed_proxy_immediate'); p.wait_for_function("document.querySelector('#m img').complete && document.querySelector('#m img').naturalWidth>0",timeout=6000); settled=p.eval_on_selector('#m img','i=>({src:i.currentSrc,complete:i.complete,nw:i.naturalWidth})'); second=pdf(p,out,'delayed_proxy_settled'); p.close()
    assert 'red.svg' in admitted['src'] and admitted['complete']; assert not immediate['complete'] and immediate['nw']==0 and first['red_pixels']<100 and first['blue_pixels']<100 and first['print_ms']<1000; assert 'blue-delay.svg' in settled['src'] and second['blue_pixels']>10000
...
def main() -> int:
    ...
    result={'schema':1,'coordinate':'C09','verdict':'ARTIFACT-COVERED / FINDING + CONTROLS','owners':['P0-004','P0-070','P0-075','P1-003','P1-187'],'browser':version,'source_contract':contract,'tests':tests}
