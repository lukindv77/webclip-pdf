...
check('S09 locator reads current config', () => has(SOURCE, 'const config = await getYandexConfig();'));
check('S10 locator reads current account UID', () => has(SOURCE, 'const currentAccountUid = await getCurrentYandexAccountUid(operationId);'));
