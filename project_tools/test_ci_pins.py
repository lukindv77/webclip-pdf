...
GOOD_S0B_JOB = """
  p1-231-source-generation-authority:
    ...
    steps:
      ...
      - name: Verify P1-231 S0-G evidence settlement
        env:
          EXPECTED_SHA: ${{ github.event_name == 'pull_request' && github.event.pull_request.head.sha || github.sha }}
        run: node project_tools/release_evidence_settlement.js --candidate "$EXPECTED_SHA"
"""
...
def main() -> None:
    ...
    no_s0g_settlement = GOOD_REPOSITORY_INTEGRITY_WITH_S0B.replace(
        '        run: node project_tools/release_evidence_settlement.js --candidate "$EXPECTED_SHA"\n',
        "        run: node --version\n",
    )
    lane_errors = module.evaluate_repository_integrity_s0b_lane(
        {module.REPOSITORY_INTEGRITY_WORKFLOW: no_s0g_settlement}
    )
