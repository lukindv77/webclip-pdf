...
INDEX_HEADER = "# Research family navigation index\n\n"
...
def make_fixture(root: pathlib.Path) -> tuple[pathlib.Path, pathlib.Path]:
    ...
    (docs / "RESEARCH_DELTA_INDEX.md").write_text(INDEX_HEADER, encoding="utf-8")
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (docs / "RESEARCH_DELTA_ONE.md").write_text("P1-001\n", encoding="utf-8")
        (docs / "RESEARCH_DELTA_TWO.md").write_text("P1-003\n", encoding="utf-8")
        assert_error(hygiene.inspect_tree(root), "more than one temporary RESEARCH_DELTA")
        (docs / "RESEARCH_DELTA_ONE.md").unlink()
        (docs / "RESEARCH_DELTA_TWO.md").unlink()

        stage_a = "RESEARCH_ALPHA_STAGE2_2026-09-01_EVIDENCE.md"
        (docs / stage_a).write_text("checkpoint\n", encoding="utf-8")
        assert_error(hygiene.inspect_tree(root), "not indexed")
        (docs / "RESEARCH_DELTA_INDEX.md").write_text(INDEX_HEADER + f"- `{stage_a}`\n", encoding="utf-8")
        assert hygiene.inspect_tree(root) == []

        stage_b = "RESEARCH_BETA_STAGE2_2026-09-01_EVIDENCE.md"
        (docs / stage_b).write_text("checkpoint\n", encoding="utf-8")
        (docs / "RESEARCH_DELTA_INDEX.md").write_text(
        ...
        legacy = "RESEARCH_VIEWPORT_ENVIRONMENT_FIDELITY_STAGE2_2026-08-30_EVIDENCE.md"
        assert legacy in hygiene.LEGACY_STAGED_FILES
        (docs / legacy).write_text("legacy checkpoint\n", encoding="utf-8")
        (docs / "RESEARCH_DELTA_INDEX.md").write_text(INDEX_HEADER + f"- `{legacy}`\n", encoding="utf-8")
        assert hygiene.inspect_tree(root) == []

        legacy_extension = "RESEARCH_VIEWPORT_ENVIRONMENT_FIDELITY_STAGE4_2026-09-02_EVIDENCE.md"
