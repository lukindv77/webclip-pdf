...
const MARKER_KEY = '__c46ForcedWorkerRestartProbe';
...
function heavyDocument() {
  ...
  for (let i = 1; i <= 500; i += 1) {
    blocks.push(`<section class="page"><h2>C46_WORKER_RESTART_${i}</h2><p>${'forced service worker restart during guarded production print '.repeat(16)}</p></section>`);
  }
  return `<!doctype html><meta charset="utf-8"><title>C46 worker restart heavy fixture</title><style>html,body{margin:0}body{font:16px sans-serif}.page{box-sizing:border-box;min-height:980px;padding:42px;break-after:page}</style>${blocks.join('')}`;
}
...
async function startServer() {
  ...
  const recovery = '<!doctype html><meta charset="utf-8"><title>C46 worker restart recovery fixture</title><h1>C46_WORKER_RESTART_RECOVERY</h1><p>unchanged production generation after forced worker restart</p>';
...
function makeExtension(s) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'c46-worker-restart-'));
...
try {
  ...
  const verdict = report.oldAttachedAfterRestart
    ? 'STALE_DEBUGGER_SESSION_SURVIVED_WORKER_RESTART'
    : 'DEBUGGER_SESSION_CLEARED_ON_FORCED_WORKER_RESTART';
  console.log('C46_WORKER_RESTART_JSON=' + JSON.stringify({
    source, browserVersion, extensionId, closedWorkerTargetId, newWorkerTargetId,
    closeResult, armed, verdict, report
  }));
} finally {
  try { cdp?.close(); } catch {}
