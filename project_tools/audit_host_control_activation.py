...
(() => {
  ...
  globalThis.chrome = {
    runtime: {
      id: 'webclip-audit-extension',
      getURL: (path='') => `chrome-extension://webclip-audit-extension/${path}`,
      ...
      sendMessage(message) {
        ...
        if (type === 'WEBCLIP_GENERATE_PDF') return Promise.resolve({ ok: true, filename: 'p0-067-audit.pdf', operationId: message.operationId || 'audit-op', journalWarning: '' });
...
def main() -> int:
    ...
    print("P0-067 host control activation audit: FINDING reproduced")
    return 0
