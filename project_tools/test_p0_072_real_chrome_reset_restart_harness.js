...
ok(harness.includes("async function verifyKilledProfileReceipt"), 'case B has an independent killed-profile receipt witness');
ok(harness.includes("fsp.cp(sourceIndexedDb, witnessIndexedDb"), 'case B copies killed-profile IndexedDB before original relaunch');
ok(harness.includes("String(item?.operationId || '') === String(operationId || '')"), 'killed-profile witness binds exact durable operation identity');
ok(harness.includes("killed-profile durable superseded receipt witness"), 'case B requires admitted superseded receipt in killed-profile IndexedDB');
ok(harness.includes("terminal-onChanged-before-page-probe"), 'case B permits only explicit terminal onChanged consumption before page probe');
