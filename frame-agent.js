(() => {
  ...
  const state = { phase: 'idle', mode: 'include', includes: new Map(), excludes: new Map(), nextId: 1, printStyle: null, changedAttrs: [] };
  ...
  function clear() {
    ...
    state.includes.clear(); state.excludes.clear(); sendState();
  }
  function addInclude(el) {
    const key=exact(state.includes,el); if(key){el.removeAttribute(INCLUDE_ATTR);state.includes.delete(key); cleanupExcludes();sendState();return;}
  ...
  function addExclude(el) {
    const key=exact(state.excludes,el); if(key){el.removeAttribute(EXCLUDE_ATTR);state.excludes.delete(key);sendState();return;}
  ...
  chrome.runtime.onMessage.addListener((msg,sender,sendResponse)=>{
    ...
    Promise.resolve().then(async()=>{switch(String(msg.command||'')){
      case'start':start(msg.mode);return{ok:true,snapshot:snapshot()}; case'set-mode':state.mode=msg.mode==='exclude'?'exclude':'include';return{ok:true}; case'clear':clear();return{ok:true}; case'stop':stop(Boolean(msg.clear));return{ok:true}; case'get-state':return{ok:true,snapshot:snapshot(),phase:state.phase}; case'restore':return{ok:true,result:restoreOne(msg.kind,msg.locator)}; case'prepare-print':return preparePrint(); case'restore-print':return restorePrint(); default:return{ok:false,error:'Unknown frame-agent command'} }
