importScripts('public-suffix.js', 'journal-import-stream.js', 'journal-text-filter.js');
...
async function bindPendingLocalDownloadIntent(intentKey, downloadId) {
  ...
  try {
    return await runIndexedDbTransactionBounded(db, JOURNAL_PENDING_DOWNLOAD_STORE, 'readwrite', 'Привязка downloadId к intent локальной загрузки', ({ store, setResult, fail }) => {
      const pending = store(); const req = pending.get(key);
      req.onsuccess = () => { try { const intent = req.result; if (!intent) { setResult(null); return; } const bound = { ...intent, downloadId: id, kind: 'download', updatedAt: Date.now() }; pending.delete(key); pending.put(bound); setResult(bound); } catch (e) { fail(e); } };
