# Durable research evidence — cross-origin frame print media / selected-only geometry — 2026-08-30

Canonical status and single-owner authority remain exclusively in `RESEARCH_REGISTRY.md`. This document preserves a **50-block** source-first deep-research tranche focused on the physical cross-origin iframe representation rendered by Chromium after WebClip has already admitted a save.

Researched fresh source baseline: `main` at `3c08e7f1bca2990acb36497f91afd87bc490be04`.
...
Immediately before allocating P1-229, the research re-read:

- current `RESEARCH_REGISTRY.md` (late stream occupied only through P1-228);
- current `RESEARCH_DELTA_INDEX.md` and recent selection/save-freeze evidence;
- `RESEARCH_FAMILY_FRAME_PERMISSION_IDENTITY_EVIDENCE.md`;
- `RESEARCH_FAMILY_PDF_PRINT_OFFSCREEN_EVIDENCE.md`;
...
This tranche changes research documentation only. Managed Chromium proves the media/geometry mechanics and the resulting PDF output in deterministic fixtures; it does **not** upgrade current release readiness or claim real unpacked-extension optional-host-permission regression.
