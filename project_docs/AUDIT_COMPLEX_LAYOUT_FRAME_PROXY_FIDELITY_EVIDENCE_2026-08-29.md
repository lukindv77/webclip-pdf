# Durable audit evidence — complex selected layout / flattened iframe representation fidelity — 2026-08-29

Canonical status and single-owner authority remain exclusively in `AUDIT_REGISTRY.md`. This document preserves source proof, managed-Chromium semantic reproductions, positive controls, dedup decisions and external web-clipping comparison from a 38-block deep-audit tranche. Runtime, `manifest.json`, version, build/tag/release state and release readiness are unchanged.

Audited fresh source baseline: `main` at `ca906a363e88f5ce78281633370dbb644c080cae`.
...
These are deliberately **not** classified as WebClip defects. Future real HTTP(S)/unpacked-browser testing may revisit them, but audit evidence must not promote an unproven hypothesis merely because it fits the same conceptual category.
...
- P1-003 was already reopened ACTIVE by the immediately preceding audit tranche and now explicitly covers the actual selected visual resource graph.
...
This is audit-only evidence. Managed Chromium semantic probes do not substitute for:
