# Durable audit evidence — selected-only CSS cascade authority — 2026-08-30

Canonical status and single-owner authority remain exclusively in `AUDIT_REGISTRY.md`. This document preserves a **28-block** source-first / managed-Chromium audit tranche focused on whether page-owned CSS can defeat WebClip's live-DOM selected-only Include/Exclude filtering during physical PDF generation.

Audited fresh source baseline: `main` at `fd852e63ef34c4600285667928dee17cefcdb83d` (P1-229 audit already merged; runtime unchanged).
...
- **P1-229 ACTIVE — acceptance refinement/correction.** The preceding media/geometry audit proved that a media-independent child filter fixes the `screen`/`print` mismatch in a neutral fixture. This tranche explicitly narrows that positive control: an ordinary author stylesheet is not by itself a complete P1-229 fix because page `!important` cascade can still override the selected-only rule. “WebClip-owned” must mean effective representation authority, not merely a `<style>` element inserted by WebClip into page author origin.
...
A cross-origin-like positive/correction fixture then used the media-independent child filter proposed only as a media control in the P1-229 audit. An unselected child and an Exclude carrying inline `display:block!important` both printed. Therefore “remove `@media print`” is insufficient as full P1-229 closure.
...
## Blocks 13–18 — negative controls and audit restraint
...
The P1-229 audit intentionally used a neutral fixture to prove that keeping worker media=`screen` while making WebClip child filtering media-independent resolves the screen-vs-print mismatch.
...
Current `AUDIT_CAPTURE_REPRESENTATION_DEPENDENCY_EVIDENCE.md` already states:
...
This tranche changes audit documentation only. No runtime tests are claimed executed by writing this file; managed Chromium fixture probes are engineering evidence. `manifest.json` remains `0.9.8`; no build/tag/release is created.