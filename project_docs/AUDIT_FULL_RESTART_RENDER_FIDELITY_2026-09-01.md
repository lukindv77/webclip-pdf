...
For coverage interpretation this refines C06 and C14 and also supports the cross-cutting resource/generation regions C21/C36 when they are audited directly later. It does not by itself complete those later coordinates.
...
The accepted run independently executed the current `project_tools/audit_p0_071_print_render_guard.py` against Chrome 152, with only its stale harness source-string check adapted to the current grouped bootstrap spelling.
...
| C12 | `L4-PARTIAL / POSITIVE CONTROL` | Video poster physically preserved; actual played/current video frame remains unaudited in this fresh campaign. |
...
`AUDIT_REGISTRY.md` remains unchanged. This is evidence/coverage only:
