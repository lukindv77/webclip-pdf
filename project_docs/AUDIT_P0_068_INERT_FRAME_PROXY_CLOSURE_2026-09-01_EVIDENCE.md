...
The guard is intentionally source-bound to the single audited `.cloneNode(true)` call in current `content.js`. In the extension isolated world it replaces deep Element cloning with a manual inert mirror builder. Host-page main-world `Node.prototype.cloneNode` remains native.
...
- current `content.js` retains exactly one audited deep-clone interception boundary.
...
The current-Chrome harness is `project_tools/audit_p0_068_inert_frame_proxy.py`.
...
After accepted evidence head `9e44a66340f06a62309145d17e7b4369aeffb35a`, the temporary evidence workflow is deleted and only durable audit/status/index documentation may change before delivery. Runtime guards, wiring, deterministic regression and physical harness must remain byte-equivalent for this Chrome evidence to stay applicable.
