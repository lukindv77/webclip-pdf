# Research change workflow — WebClip

Этот документ задаёт обязательный жизненный цикл новых research findings и изменений существующих P-owner после консолидации research evidence.

`RESEARCH_REGISTRY.md` остаётся единственным current authority по P-кодам, owner и status. Family evidence хранит доказательную базу; GitHub Issue/PR — рабочий процесс, но не заменяет registry.

Организация каждой инструментальной research-сессии дополнительно и обязательно подчиняется `SESSION_EXECUTION_AND_INTERRUPTION_SAFETY_POLICY.md`: перед началом сообщается exact состояние незавершённой работы и план/риски сессии, объём рассчитывается на максимально полезное использование всего инструментального окна, а substantive progress сохраняется этапами в durable checkpoints так, чтобы неожиданное прерывание не требовало реконструкции работы из чата или локального ephemeral состояния.

Выбор и приоритизация крупных deep-research user operations/surfaces дополнительно и обязательно подчиняются `RESEARCH_EXTERNAL_USER_INTENT_RESEARCH_POLICY.md`. Deep research должен регулярно сверять свою User Intent / Operation Map с актуальным внешним опытом похожих и пересекающихся продуктов: official developer materials/demos, GitHub projects/issues/discussions, user forums/Reddit, reviews/comparisons и web-archiving practices. Внешний опыт является evidence-input для того, **что** нужно аудировать и с каким приоритетом, но не автоматически меняет product contract WebClip и не создаёт P-owner без обычной root-cause/admission процедуры.

Для текущего основного PDF-режима `WEBCLIP_PDF_FIDELITY_CONTRACT.md` является обязательной PDF-specific acceptance authority. Deep research не должен оценивать PDF по неопределённому критерию «похоже на страницу»: ordinary-scroll completeness, user-reached boundary для scroll-triggered dynamic content, spoiler materialization, hover exclusion, responsive/resource identity, temporal/focus state, pagination и truthful degradation проверяются относительно явно принятого contract.

`RESEARCH_COVERAGE_CAMPAIGN_POLICY.md` является обязательной методикой систематического deep research. Она разделяет coverage completeness и finding closure, задаёт Coverage Matrix по end-to-end boundaries/surface families, evidence ladder L1–L5, controls, tranche envelope, root-cause saturation, campaign ranking, closure re-research и project-wide completion gates. Наличие большого P-registry или большого количества research blocks не является доказательством полной coverage. Current initial reconstruction хранится в `RESEARCH_COVERAGE_RECONSTRUCTION_2026-08-30.md` и должна обновляться/заменяться последующими durable coverage checkpoints.

## 1. Начало работы

1. Fresh-fetch `main` и зафиксировать exact baseline SHA.
2. Проверить `RESEARCH_REGISTRY.md`, `RESEARCH_DELTA_INDEX.md`, соответствующий `RESEARCH_FAMILY_*_EVIDENCE.md`, `RESEARCH_HISTORY_INDEX.md`, current Coverage Matrix/reconstruction и Git history.
3. Перед выбором нового крупного deep-research tranche проверить freshness текущего external user-intent baseline по `RESEARCH_EXTERNAL_USER_INTENT_RESEARCH_POLICY.md`; если baseline устарел или появился существенный новый peer-product/platform/user-intent signal, сначала выполнить и durably сохранить необходимый delta-scan/research refresh.
4. Выбирать следующий крупный tranche из nonterminal/high-risk coverage gaps согласно `RESEARCH_COVERAGE_CAMPAIGN_POLICY.md`, а не только по наличию очередного интересного edge case. До старта определить boundaries, required evidence, controls, variant classes и termination envelope.
5. Если tranche затрагивает текущий основной PDF fidelity/completeness, сформулировать research questions и acceptance относительно `WEBCLIP_PDF_FIDELITY_CONTRACT.md`, включая применимые transformation class и границу admitted/user-reached state.
...
Для анализа рекомендуется GitHub Issue по шаблону `Research finding`.
...
- acceptance criteria;
- coverage cells/surfaces и required evidence level, если finding возник в systematic deep-research campaign.

Issue может существовать до присвоения P-кода. **Резервирование номера происходит только после записи в `RESEARCH_REGISTRY.md`.**
...
## 5. Machine-readable research impact

`project_tools/check_pr_change_contract.py` проверяет actual base/head diff PR, а не доверяет только описанию автора.

Если PR меняет runtime или canonical research/evidence, в PR body выбирается ровно одно:

- `research-impact: none` — P-owner/status/acceptance contract не меняется;
- `research-impact: owner` — перечислены затронутые P-коды и durable evidence меняется в том же PR.

Для **любого runtime change** дополнительно обязателен конкретный `research-rationale:`. Он должен объяснять либо почему существующие P-owner/invariants действительно не затронуты, либо каким образом затронуты объявленные owner. Пустой, placeholder, `none` или `n/a` rationale не принимается.

Для `research-impact: owner` действуют дополнительные инварианты:
...
Изменение `RESEARCH_REGISTRY.md` требует в том же PR второго durable family/history evidence файла. Это не позволяет registry стать единственным местом, где существует новая acceptance-деталь.
...
## 7. Closure re-research

После implementation owner нельзя проверять только старый reproduction. Согласно `RESEARCH_COVERAGE_CAMPAIGN_POLICY.md` выполняется Closure Sweep соответствующего region Coverage Matrix.
...
Coverage outcome и P-status различаются: `ARTIFACT-COVERED / FINDING` может быть terminal research cell при ACTIVE owner.

## 9. Evidence после merge

После merge current truth должен быть восстанавливаем из:

1. `RESEARCH_REGISTRY.md` — owner/status;
...
## 10. Deep-research completion boundary

Project state `DEEP-RESEARCH-COVERAGE-COMPLETE` допустим только по gates `RESEARCH_COVERAGE_CAMPAIGN_POLICY.md`: все CORE families triaged, все material relevant cells terminal с required evidence либо explicit external/limitation boundary, все findings имеют root-cause ownership и нет unexplained NOT-RESEARCHED областей.

Этот статус не означает, что все findings исправлены. `DEEP-RESEARCH-CRITICAL-CLOSURE-COMPLETE` и `RELEASE-READY` являются отдельными более сильными состояниями.

## 11. Release boundary

Research implementation и release readiness — разные состояния. Release-кандидат дополнительно проходит `.github/workflows/release-gate.yml` и правила `RELEASE_READINESS.md`.
