...
- branch: `research/cycle2-scroll-triggered-animation-2026-09-01`;
...
- probe: `project_tools/research_scroll_triggered_animation.py`;
...
```css
.trigger-subject {
  trigger-scope: --research-trigger;
  timeline-trigger: --research-trigger view() contain / cover;
}
.trigger-visual {
  animation: research-reveal 220ms linear both;
  animation-trigger: --research-trigger play-forwards reset;
}
```

Chrome 152 confirms both required syntax paths:

- `CSS.supports('timeline-trigger: --research-trigger view() contain / cover') = true`;
- `CSS.supports('animation-trigger: --research-trigger play-forwards reset') = true`;
- the basic `play-forwards play-backwards` form is also supported.

The `reset` deactivation action is valuable for this research because it makes a geometry-induced active→inactive transition observable immediately and deterministically: animation progress returns to zero and pauses.
...
`RESEARCH_REGISTRY.md` remains unchanged and remains the sole P-code/status authority.
...
5. only then decide whether `DEEP-RESEARCH-COVERAGE-COMPLETE` is warranted for Cycle 2.

No build, tag or GitHub Release is implied by this research tranche. `RELEASE_READINESS.md` remains unchanged / `NOT READY`.