# Durable audit evidence — local prepare budget / task lifetime — 2026-08-30

Canonical current P-code owner/status authority remains exclusively in `AUDIT_REGISTRY.md`. This document preserves a completed **56-block** source-first correctness/performance audit of local PDF preparation work, diagnostic acquisition and resource-task lifetime.
...
`frame-agent.js` stores at most 100 image references for resource handling, but creates `root.querySelectorAll('img')` before stopping pushes at 100. Historical `AUDIT_DELTA_FRAME_AGENT_RESOURCE_SCAN_BUDGET_2026-08-28.md` already owns that remote scan-budget issue.
