# Durable audit evidence — SelectionSnapshot privacy revalidation — 2026-08-30

Canonical status and single-owner authority remain exclusively in `AUDIT_REGISTRY.md`. This is an interruption-safe source-first checkpoint from fresh `main` at `a899ae3d22c365010a663389dc07844b34c86f77`.
...
1. The general live-DOM / `beforeprint` mutation window is already losslessly preserved in `AUDIT_FAMILY_PDF_PRINT_OFFSCREEN_EVIDENCE.md` under the existing frozen-representation owners.
...
3. The same-origin flattened-frame relative SVG `<use href>` / `<picture><source srcset>` provenance defects are already explicitly covered by the 38-block `AUDIT_COMPLEX_LAYOUT_FRAME_PROXY_FIDELITY_EVIDENCE_2026-08-29.md` under P1-187.
