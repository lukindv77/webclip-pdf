...
2. current `project_tools/audit_selection_restore_physical_pdf.py` for C02;
3. current `project_tools/audit_main_content_physical_pdf.py` for C03;
4. current `project_tools/audit_pdf_cache_generation_authority.py` for exact cache-generation authority.

The two older physical audit scripts still contained the legacy Playwright call `BrowserContext.new_page(viewport=...)`, which Playwright 1.55 no longer accepts. The temporary workflow transformed only those audit-script calls in the runner checkout to `new_page()` plus `set_viewport_size()` while preserving each original indentation. No product file was rewritten by that compatibility step. The first two failed development executions were therefore harness/orchestration failures and are not evidence. Only run `33500051622` is accepted.
...
`project_tools/audit_pdf_cache_generation_authority.py` bound its race schedules to the exact current source and passed on the accepted head.
...
No new P-code is warranted by this tranche. All reproduced failures map to existing owners/root regions. No DONE owner is reopened by this evidence. `AUDIT_REGISTRY.md` is unchanged.
