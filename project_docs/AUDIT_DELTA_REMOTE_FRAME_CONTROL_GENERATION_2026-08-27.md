...
Date: 2026-08-27
Source-of-truth `main` immediately before write: `aa7978e106a12b5cf0f1e2740eb26484cb2df973`.
...
- **P1-004** — feature-level cross-origin iframe support umbrella. P1-200 is a concrete same-document command-order authority defect.
...
Introduce a document-bound **selectionSessionId / selectionGeneration** plus monotonic control sequence for remote frame commands.
...
8. Child state responses/events carry generation so top does not accept stale snapshots as current.
9. Navigation/document replacement still invalidates everything per P1-171.
10. Print generation P1-199 is layered on top of the current selection generation and remains separately fenced.
...
5. stale child `STATE` event from A cannot replace B snapshot/count UI;
...
9. print prepare/restore still uses its independent P1-199 generation without reopening selection-control races.

Real unpacked Chrome QA should include rapid Stop/Start and Include↔Exclude toggles with an intentionally delayed optional-permission/frame relay.
