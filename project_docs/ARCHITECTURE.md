...
Large Journal import is bounded end-to-end. Local files are staged in `WebClipOffscreenTransfers` as 1 MiB Blob chunks; Yandex backup downloads are streamed by the offscreen document directly into the same Blob-chunk format instead of `response.text()`. `journal-import-stream.js` parses the export envelope incrementally and never builds the root backup object or the full `journal.entries[]` array. During the same byte pass, `journal-import-digest.js` computes incremental SHA-256 without retaining the whole backup. Preview returns a strict receipt bound to receipt version, explicit `replace` mode, staging key/generation, source, operation id, content digest, entry count, export time and expected Journal revision; the Journal confirmation displays the exact digest.

Preview also creates one durable `journalImportLease` checkpoint in `WebClipJournal.meta`. Its exact v1 value binds the worker-issued receipt to a rotating `leaseToken`, one `ownerSessionId`, creation/update timestamps, a renewable two-minute `leaseExpiresAt`, and a staging-generation-based two-hour `hardExpiresAt`. The live Journal page renews only its exact token/owner. A second page cannot preview, resume, cancel or discard that checkpoint while the short lease is current.

After a browser/page restart, no destructive action resumes automatically. When the short lease expires, `journal.html` may show an explicit resume-or-cancel confirmation. Resume atomically rotates token/owner, then re-reads and re-hashes the exact staged bytes, captures a fresh Journal revision and requires a second nine-digit destructive confirmation. Cancel removes only the expired checkpoint and its raw staging. Hard-expired, missing or generation-invalid checkpoints are the only automatic reclamation cases.

Generic transfer TTL cleanup protects the exact hard-live staging generation. Corrupt or unparseable checkpoint metadata fails closed by protecting all Journal-import manifest/chunk rows rather than guessing that they are orphaned. The final replacement re-reads and normalizes entries in bounded batches, then one readwrite transaction spanning `entries`, `meta`, pending recovery stores and `importStaging` checks exact lease authority first, compares the expected revision second, deletes the lease, and only then begins destructive writes. Lease/receipt/revision mismatch aborts without changing Journal or its recovery checkpoints; raw staging is retained for the current owner after failed authority checks and removed only after successful commit. Legacy inline full-object runtime import remains disabled. P1-215 is closed; reconciliation of already-admitted pending side effects remains separate C44/P0-072 work.
