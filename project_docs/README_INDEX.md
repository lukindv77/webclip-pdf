...
5. `AUDIT_COVERAGE_CYCLE2_MATRIX_2026-08-31.md` — current Cycle-2 Coverage Sweep / Matrix v2: C01…C46 re-triage, 31 terminal carry-forward families, 15 Change-Impact revalidation families; PD2 уже закрыт как `ARTIFACT-COVERED / FINDING`, остальные PD1/PD3–PD6 остаются в очереди, следующий tranche T2.
6. `AUDIT_CYCLE2_T1_VIEW_TRANSITION_RENDER_CUT_2026-08-31.md` — T1/PD2 current-Chrome physical evidence: document/element/concurrent/Shadow View Transition midpoint не сохраняется в PDF; finding остаётся под P0-070/P0-075/P0-004 без нового P-кода.
7. `AUDIT_COVERAGE_CYCLE2_KICKOFF_2026-08-31.md` — second full deep-audit campaign start: fresh baseline, platform/external Change Impact variants, initial risk ranking и `DEEP-AUDIT-IN-PROGRESS` state.
8. `AUDIT_EXTERNAL_USER_INTENT_BASELINE_2026-08-31.md` — current substantive external user-intent/product-discovery baseline for Cycle 2, включая fresh peer/community/archive/browser-platform scan.
9. `AUDIT_COVERAGE_FINAL_SYNTHESIS_2026-08-31.md` — final synthesis **previous Cycle 1**; его `DEEP-AUDIT-COVERAGE-COMPLETE` остаётся исторически истинным для завершённой первой кампании, но не означает completion текущего Cycle 2.
10. `AUDIT_COVERAGE_RECONSTRUCTION_2026-08-30.md` — Cycle-1 family-level C01…C46 Coverage Matrix и последовательность targeted revalidation tranches.
11. `AUDIT_COVERAGE_CAMPAIGN_POLICY.md` — обязательная методика systematic deep audit, evidence ladder L1–L5 и project-wide completion gates.
12. `AUDIT_DELTA_INDEX.md` — навигация по полностью консолидированным audit families.
13. `AUDIT_CHANGE_WORKFLOW.md` — жизненный цикл нового finding / P-owner / implementation / evidence / PR / status transition.
14. `TEST_STATUS.md` — текущая правда о тестах/release gate; historical PASS не является current rerun.
15. `RELEASE_READINESS.md` — machine-readable current release-readiness declaration; `NOT READY` является нормальным WIP-состоянием.
16. `PROJECT_OVERVIEW.md` — назначение проекта и основные инварианты.
17. `USER_REQUIREMENTS.md` — актуальные требования и явно отмеченные superseded правила.
18. `ARCHITECTURE.md` — компоненты, current implemented behavior и явно обозначенные open audit boundaries.
19. `DECISIONS_AND_RATIONALE.md` — архитектурные решения, включая помеченные superseded historical decisions.
20. `DATA_MODELS.md` — IndexedDB/storage/snapshot/receipt models.
21. `TEST_PLAN.md` — regression plan и Git-first recovery gate.
22. `BUILD_AND_RECOVERY_RULES.md` — release/recovery/provenance architecture.
23. `GITHUB_WORKFLOW.md` — PR-first working policy, accepted `protected=false` posture, integrity/release gates.
24. `RELEASE_HISTORY_INDEX.md` — inventory и retention policy исторических evidence-bearing GitHub Releases/tags.
25. `CHANGELOG_AND_RATIONALE.md` — история функциональных изменений.
26. `ASSISTANT_NOTES_AND_LIMITATIONS.md` — технические ограничения.
...
- `AUDIT_COVERAGE_CYCLE2_MATRIX_2026-08-31.md` — current Cycle-2 Coverage Matrix. First sweep re-triaged all C01…C46: 31 families retain required historical evidence under Change Impact, while 15 contain newly stable Chrome 146–152 renderer variants. T1/PD2 View Transition render-cut revalidation is now `ARTIFACT-COVERED / FINDING`; C14/C18/C33/C35 remain family-nonterminal only because PD3/PD4/PD1 are still pending. PD7 Chrome 153 remains a beta watch;
