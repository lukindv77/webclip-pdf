...
| P1-164 | ACTIVE | Standalone revoke, delete-time `revoke + keep-file`, and explicit two-effect `revoke + Trash` use durable phase/identity/Journal authority; admitted unpublish or move is never replayed. Qualification tooling can now attest exact running-extension source and passively prove matching destructive request emission, but provider-state causality and the real authorized Yandex/browser matrix remain required. |
