...
| P1-179 | IMPLEMENTED / RELEASE-REGRESSION | Backup lease, pending checkpoint and scheduler success/failure/last-path state are immutable account/root namespaces; legacy flat state cannot become current authority; same-account old-root checkpoints reconcile read-only on exact historical paths before fresh current-root provisioning. Applicable real Chrome/Yandex regression remains separate. |
| P1-180 | IMPLEMENTED / RELEASE-REGRESSION | Bulk local clear/replace operations use exact-revision worker-issued disclosure of known public Yandex links and explicitly preserve no-hidden-mass-unpublish semantics; stale disclosure fails closed. Applicable real Chrome/Yandex release regression remains separate. |
