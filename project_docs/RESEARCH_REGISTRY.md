...
| P0-069 | ACTIVE | Deleting a Journal entry with a public Yandex link requires explicit publication outcome; preserve and durable `revoke + keep-file + exact Journal delete` are implemented, while `revoke + Trash` remains fail-closed pending a second-remote-effect protocol and live evidence. |
...
| P1-164 | ACTIVE | Standalone revoke and bounded delete-time `revoke + keep-file` now share checkpoint-before-unpublish, observation-only restart reconciliation, exact identity continuity and a persisted local completion; live Yandex qualification and `revoke + Trash` composition remain. |
