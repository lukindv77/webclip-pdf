...
| C44 | OPEN — L4 partial; digest/revision repair complete; lease/checkpoint/native/merge/remote remain | Add a bounded renewable lease and explicit restart resume/cancel/orphan reclamation for transfer/normalized staging; then preserve, migrate or explicitly quarantine admitted side-effect checkpoints instead of clearing them as cancellation evidence. Re-run the accepted restart matrix. Record merge as unsupported or specify/test it separately. | P0-013, P0-072, P1-207, P1-215; P1-194 supporting; P0-077 DONE positive | B2/B6/B7/B8/B9; next local post-remediation L4 is lease/restart ownership, native Save As and authorized remote context remain L5 |
...
Execution stays sequential unless an earlier row exposes a cross-boundary blocker. **C41 remains blocked on real native/restart L5; C42 remains blocked on an explicitly authorized Yandex test account/root; C43 remote shares the C42 external prerequisite. C44 now has accepted real-UI SHA-256/staging-generation receipt binding and in-transaction Journal revision CAS; its next local work is bounded lease/restart ownership followed by checkpoint reconciliation, while native Save As, merge decision and remote recovery remain open. C45 has accepted degraded-resource physical-PDF L4 evidence and a P0-066 failed-resource URI-query finding; GUI/native activation and post-remediation proof remain L5 open. C46 has accepted production real-unpacked debugger/physical-PDF/normal-detach L4 evidence; its native permission, incognito, revoke/regrant, browser-initiated detach and restart boundaries remain L5 open.** External-account/user-owned UI work must not be simulated as L5; unavailable external prerequisites remain explicit blockers rather than inferred success.


## Fresh continuation checkpoint — focused C44 Backup / import / recovery

`RESEARCH_FULL_RESTART_C44_BACKUP_IMPORT_RECOVERY_2026-09-03.md` now records three accepted complementary tranches. The latest post-remediation production-path tranche uses canonical base `e4fdd7ba62d4d4d810be39f21e6e7c42e9ee3c0e`, Chrome `152.0.7977.54`, workflow run `33790752301`, job `100766491470`, exact accepted workflow head `3cbf4a2efe22034fdf4a8b61541f5b167a9df476`, conclusion SUCCESS, result SHA-256 `4a90e0177463db855092c48ec7ee8610b19af498d9e9405f93cc9f7bc8529996` and retained machine-receipt artifact id `9907225369`. The pre-remediation real-UI discovery run `33787642007` and earlier physical IDB/race run `33723881258` remain historical supporting evidence.

The real unpacked journal page now physically covers a strict preview receipt bound to SHA-256, staging generation, `replace` mode and expected Journal revision. A same-key/same-generation/same-size valid-byte retarget fails on digest mismatch; a separate concurrent Journal update fails the in-transaction revision CAS; both paths preserve Journal and every pending row. A clean unchanged import succeeds.

Full restart still preserves transfer rows and Journal/recovery state but does not restore confirmation, selected File or operation id. A fresh retry succeeds and consumes its new staging while the abandoned pre-restart generation remains orphaned. Earlier fixed-TTL/lease controls remain causal feasibility evidence for the next repair.

Duplicate/root-cause reconciliation keeps **P0-013 / P0-072 / P1-207 / P1-215 ACTIVE**, with **P1-194 ACTIVE** supporting storage-durability semantics; **P0-077 DONE** remains positive only. The runtime and deterministic suite change, but no new P-code or Registry status change is warranted; manifest remains `0.9.8` and release remains NOT READY. C44 advances to `L4-REVALIDATED / PARTIAL/FINDING + POSITIVE/REAL-UNPACKED/PREVIEW-RECEIPT/SHA-256-BINDING/STAGING-GENERATION/REVISION-CAS/FAIL-CLOSED/CLEAN-IMPORT/FULL-BROWSER-RESTART CONTROLS + RECOVERY-CLEAR/RESTART-RESUME-LOSS/ORPHAN-STAGING FINDINGS; LEASE/CHECKPOINT/NATIVE-SAVE-AS/MERGE/REMOTE-L5 OPEN (P0-013, P0-072, P1-207, P1-215; P1-194 supporting; P0-077 positive)`.

C44 remains OPEN. The completed local task is immutable backup digest/staging/revision/mode binding. The next local exit task is a bounded renewable lease plus restart resume/cancel/orphan reclamation; checkpoint reconciliation follows. Native Save As stays user-owned L5; remote recovery requires an explicitly authorized isolated Yandex context; merge must be an explicit product decision or separately specified implementation.
