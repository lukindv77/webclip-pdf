...
Current `content.js` is unchanged and still contains exactly one programmatic `.click()` call: the audited `triggerInternalClick(control)` boundary. When that page-owned activation is blocked, existing code can still apply `forcePanelVisible(panel, control)` to a linked disclosure panel that already exists in the selected DOM.
...
`DEEP-AUDIT-COVERAGE-COMPLETE` remains true while broader critical closure remains incomplete.
