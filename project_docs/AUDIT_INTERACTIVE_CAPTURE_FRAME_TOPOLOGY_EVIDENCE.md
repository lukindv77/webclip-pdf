# Durable audit evidence — interactive capture / live frame topology / renderer state — 2026-08-29

Canonical status remains exclusively in `AUDIT_REGISTRY.md`. This file is durable supporting evidence for a 24-block semantic audit tranche. Runtime, `manifest.json`, build/tag/release state and release-readiness are not changed by this evidence.

Baseline source audited: `main` at `0121f81a1caa447b2d184b22ae08ef5b1c69237b`.
...
Direct Chromium PDF controls in the same audit showed ordinary masks, border-image and pseudo-element presentation can be rendered by Chromium. Therefore this loss is introduced by WebClip flattening, not an inherent PDF limitation.
...
This is source/browser/research audit evidence only. No runtime fix is claimed. P0/P1 owners remain unresolved until implementation and their required direct verification are complete. No version bump/build/tag/release is implied.