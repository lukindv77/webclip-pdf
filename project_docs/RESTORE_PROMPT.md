...
**Свежий GitHub `main` — единственный источник истины для текущего дерева.** Никогда не восстанавливать старый snapshot поверх более нового `main` и не считать handoff/recovery archive параллельным source of truth.
...
3. Прочитать bootstrap-набор из manifest:
...
5. Читать только файлы нужного профиля и релевантные `RESEARCH_FAMILY_*_EVIDENCE.md` через навигацию `RESEARCH_DELTA_INDEX.md`. Для профиля **research** обязательно прочитать `project_docs/CONTEXT_AUTOMATION_POLICY.md`, включая постоянную границу defensive-security scope.
6. Для исторического контекста использовать `RESEARCH_HISTORY_INDEX.md`, `RESEARCH_EVIDENCE.md`, `TEST_EVIDENCE.md`, retired evidence и Git history только по необходимости.
...
всегда запускает handoff-протокол, даже если разработка, исследование, PR или обсуждение не завершены.
