...
- `AUDIT_REPLACED_RESOURCE_CONVERGENCE_2026-08-30_EVIDENCE.md` + `AUDIT_REPLACED_RESOURCE_CONVERGENCE_STAGE2_2026-08-30_EVIDENCE.md` + `AUDIT_REPLACED_RESOURCE_CONVERGENCE_FINAL_2026-08-30_EVIDENCE.md`
  ...
  56-block interruption-safe renderer-owned replaced/media/resource convergence tranche from fresh `main`. No new P-number/status transition: exact current-CDP physical proof refines P1-003 because delayed primary video media, video poster, SVG `<image>`, SVG `<feImage>`, image-input resources and still-pending stylesheet state can be absent from valid PDF bytes without entering current IMG/background/font readiness counters; an ordinary delayed IMG wait remains a positive bounded control. P1-187 is refined because deep-cloned same-origin frame media resets current paused video/audio state and relative text-track/image-input/SVG-image resources can rebase to the top document even while nested media source/poster URL copying remains a positive control. P0-070/P2-007 own exact renderer-generation/current-view versus static-media semantics; P0-004/P1-167/P0-075/P1-229 remain supporting boundaries. WebGL/object/embed/external-use inconclusive probes are explicitly rejected; `P1-230` remains unallocated.
