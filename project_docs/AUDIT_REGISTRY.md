...
|---|---|---|
| P0-077 | DONE | Full-Journal self-export and same-version restore now share one versioned envelope: 50 MiB UTF-8 / 50 MiB chars / 100,000 entries / 8 MiB per serialized entry. Worker bootstrap clamps import options, replaces the legacy 4 MiB export batch reader with an 8 MiB envelope reader, and rejects + cleans staged export receipts above the same-version byte/count boundary before local/Yandex success. Existing records are neither deleted nor truncated. See `AUDIT_P0_077_JOURNAL_RESTORE_ENVELOPE_CLOSURE_2026-09-01_EVIDENCE.md`. |
