# Canonical audit registry — WebClip
...
- **ACTIVE** — current source/audit still has an unresolved acceptance contract. Historical implementation PASS does not close it.
...
This compact rule replaces hundreds of old `REGRESSION` rows without declaring them release-tested on the current HEAD. Historical implementation/browser/test evidence is in `AUDIT_EVIDENCE.md` and `TEST_EVIDENCE.md`; exact old wording remains in Git history.
...
| P0-033 | ACTIVE | Signed Yandex transport URL is an opaque capability; current OperationLog regression retains signed pathname and must restore full path/query/fragment redaction. |
| P0-039 | ACTIVE | Unknown local-download physical outcome must not lose its only durable recovery evidence merely because Download history disappeared/TTL elapsed. |
...
| P0-048 | ACTIVE | Local-download fallback identity is ambiguous; one physical `downloadId` cannot be heuristically claimed/overwritten by multiple intents. |
...
| P0-077 | ACTIVE | WebClip must never report a successful self-generated backup/export that the same version cannot restore because byte/count/per-entry envelopes disagree. |
...
|---|---|---|
| P1-153 | DONE | Original `its.1c.ru` root-document pagination/clipping repro: real Chrome generated complete 2-page PDF after root print-flow normalization. Historical measurements are in `TEST_EVIDENCE.md` / `AUDIT_EVIDENCE.md`. Later print-safety/rollback owners do not reopen that exact clipping result. |
...
- `AUDIT_HISTORY_INDEX.md` — retractions, dedup decisions and product/security decisions;
