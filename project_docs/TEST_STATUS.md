...
P0-072 owns the rule that deleting/replacing Journal presentation is not evidence that an already admitted external effect was cancelled. The current bounded implementation covers the **Yandex remote-save checkpoint class only**.

Journal clear-all, URL/site clear and staged import-replace no longer blindly clear/prune matching `pendingRemoteSaves`. In the same Journal IndexedDB transaction as the destructive reset, remote checkpoints are classified by external phase. `admitted-unknown` and `stale-unverified` receipts survive and are durably marked `supersededByJournalReset` with reset time/scope. A pre-upgrade `prepared` row that lacks the exact P0-079 PDF-cache receipt is conservatively treated as ambiguous and also survives. An exact synthetic/pre-admission `prepared` row remains reset-cancellable, while `remote-verified` is already terminal remote truth and may be retired by reset.

The superseded marker is **not** a new live Journal authority. `appendJournalEntry` re-reads the required durable checkpoint inside the same transaction as the Journal mutation and treats a missing **or reset-superseded** receipt as fail-closed cancellation. This prevents both a live finalizer and background recovery from resurrecting old metadata into a cleared/imported Journal generation. Recovery also refuses to treat a same-id replacement Journal row as completion evidence for a superseded old remote operation. It continues remote reconciliation, and once the remote outcome is actually verified, the terminal superseded checkpoint is removed without appending the old Journal entry.

Manual retry/reactivation of an existing remote checkpoint carries forward the reset provenance rather than silently stripping it. P0-079 composition is preserved: an admitted reset-superseded checkpoint still retains its exact sealed `pdf:<generation>` through the P0-079 retention fence until remote settlement; the Journal reset marker itself does not revoke those bytes. Caller textual `operationId` is not used to decide reset retention or Journal authority.

Focused deterministic production witness: `project_tools/test_p0_072_remote_save_reset_fence.js`. It covers phase disposition, URL/site isolation, legacy ambiguity, same-id replacement, retry provenance preservation, clear/import source structure, no-resurrection append admission, terminal cleanup and P0-079 composition.

This is a **partial P0-072 implementation**, not closure. `pendingDownloads` still has the old reset semantics, and ReadLater→Upload / Delete→Trash destructive remote moves still lack the independent reset-surviving receipt required by the canonical P0-072 evidence. Authorized real Chrome/Yandex unknown/late-settlement evidence and bounded/manual-resolution behavior remain required. P0-072 therefore remains **ACTIVE** and release readiness remains **NOT READY**.
