...
S1-A now has the passive production shadow library `project_tools/release_shadow_identity.js`. It composes S0-E identity, S0-F candidate admission and S0-I exact PR-impact provenance for one exact candidate, requires the workspace HEAD to equal that candidate, requires the S0-F tuple to equal independently computed S0-E identities, and rejects PR contexts that require trusted control-plane review. The S0-I trust surface now includes S0-C/D/E/F/S1-A implementations plus the checker/workflow. Permanent Repository Integrity shadow activation remains deliberately absent from this bootstrap tranche. See `RESEARCH_P1_231_S1A_PASSIVE_PRODUCTION_SHADOW_2026-09-22_EVIDENCE.md`.
