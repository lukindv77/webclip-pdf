...
ReadLater's own pre-move Journal checkpoint increments `entryRevision` and atomically advances the receipt cursor to that committed revision, so the operation does not stale itself. Terminal ReadLater patch and Trash delete require both the existing P0-072 source matcher and the exact P0-076 cursor; stale local authority produces cancellation/non-retargeting rather than mutation of a newer row. Persisted legacy destructive receipts that predate these cursor fields retain the earlier createdAt/resource-id fallback for restart compatibility, so this tranche is intentionally **not** P0-076 closure.
...
- `project_tools/test_p0_076_destructive_receipt_cas_composition.js` — new-receipt CAS cursor binding, atomic creation/admission checks, ReadLater cursor advance, terminal non-retargeting, preserved P0-072/P1-090 remote authority and explicit legacy-receipt fallback.
...
This is **not P0-076 closure**. Local/keep delete and comment mutations use generic P0-076 CAS, and newly created ReadLater/Trash receipts now carry an exact local CAS cursor through their specialized P0-072 finalizers. The remaining boundary is legacy persisted destructive receipts without that cursor: they still use the earlier P0-072 createdAt/resource-id fallback during restart reconciliation. A later bounded tranche must choose an explicit fail-closed/retirement policy for those legacy receipts before P0-076 can be closed. P0-072/P1-090 remote-effect authority remains separate. P0-076 remains **ACTIVE** and release readiness remains **NOT READY**.
