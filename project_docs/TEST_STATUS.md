...
S0-C now has a passive production release-contract projection authority. `release_contract_inputs_v1.json` canonically owns the 11 full-RCF blob roots plus the closed unpacked-Chrome and Yandex E2E QCF projection semantics, while `project_tools/release_contract_authority.js` resolves exact candidate Git blobs and reproduces the current Chrome QCF, Yandex QCF and full RCF without interpreting receipts or mutating readiness. The manifest is control-plane only and does not change the 34-member extension package. See `RESEARCH_P1_231_S0C_PASSIVE_PRODUCTION_AUTHORITY_2026-09-21_EVIDENCE.md`.
