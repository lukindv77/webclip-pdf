...
S1-A now has the passive production shadow library `project_tools/release_shadow_identity.js`. It composes S0-E identity, S0-F candidate admission and S0-I exact PR-impact provenance for one exact candidate, requires the workspace HEAD to equal that candidate, and requires the S0-F tuple to equal independently computed S0-E identities. Current trust semantics distinguish ordinary automatic trust from protected control-plane changes: the valid pair `trustedControlPlaneReview=true / automaticClassificationTrusted=false` reports `control-plane-review-required` with `eligible=false`, while malformed trust/provenance remains structural failure. The S0-I trust surface includes S0-C/D/E/F/S1-A implementations, CI pin checker, PR checker and workflow. Permanent Repository Integrity shadow activation remains absent until the next bounded tranche. See `RESEARCH_P1_231_S1A_PASSIVE_PRODUCTION_SHADOW_2026-09-22_EVIDENCE.md` and `RESEARCH_P1_231_S1A_CONTROL_PLANE_REVIEW_RECONCILIATION_2026-09-22_EVIDENCE.md`.
