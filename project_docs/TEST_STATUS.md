...
P0-072 owns the rule that deleting/replacing Journal presentation is not evidence that an already admitted external effect was cancelled. The current bounded implementation covers the **Yandex remote-save** and **automatic local-download checkpoint** classes. ReadLater→Upload / Delete→Trash destructive remote moves remain separate unfinished P0-072 work.

For Yandex remote save, Journal clear-all, URL/site clear and staged import-replace no longer blindly clear/prune matching `pendingRemoteSaves`. In the same Journal IndexedDB transaction as the destructive reset, remote checkpoints are classified by external phase. `admitted-unknown` and `stale-unverified` receipts survive and are durably marked `supersededByJournalReset` with reset time/scope. A pre-upgrade `prepared` row that lacks the exact P0-079 PDF-cache receipt is conservatively treated as ambiguous and also survives. An exact synthetic/pre-admission `prepared` row remains reset-cancellable, while `remote-verified` is already terminal remote truth and may be retired by reset.

For automatic local download, a newly created `pendingDownloads` intent is explicitly `downloadAdmissionPhase: prepared`. Immediately before `chrome.downloads.download(...)`, the worker re-reads that exact intent in a bounded readwrite transaction and advances it to `admitted-unknown`. This is the local linearization boundary: if a concurrent Journal reset already removed the still-prepared intent, admission fails closed and Chrome is not called; if admission committed first, the reset preserves the receipt. The existing P1-146 same-worker late-settlement map, timeout semantics and restart/fallback identity reconciliation remain separate and unchanged.

Journal clear-all, URL/site clear and staged import-replace now route matching `pendingDownloads` through a phase-aware reset fence. New exact `prepared` intents are cancellable before Chrome admission. `admitted-unknown` intents, bound `download` rows, and P0-039 `unknown` / `manual-resolution` rows survive reset and are marked `supersededByJournalReset`. Pre-upgrade intents without the explicit admission field are conservatively preserved because an old build may already have invoked Chrome while retaining only an unbound intent. Late intent→DownloadItem binding carries the reset marker by row spread; the legacy existing-download alias path also copies reset provenance instead of silently stripping quarantine.

The superseded marker is **not** a new live Journal authority. `appendJournalEntry` re-reads the required durable checkpoint inside the same transaction as the Journal mutation and treats a missing **or reset-superseded** receipt as fail-closed cancellation. Remote recovery therefore cannot resurrect an old entry after clear/import, including through a same-id replacement. Local download finalization uses the same gate: once Chrome reports `complete`, a superseded checkpoint is retired without appending old Journal metadata; `interrupted` remains terminal failure and is also removed. P0-039 manual-resolution rows preserve reset provenance and remain excluded from ordinary automatic recovery until exact late settlement/manual handling.

Manual retry/reactivation of an existing remote checkpoint carries forward reset provenance. P0-079 composition is preserved: an admitted reset-superseded remote checkpoint still retains its exact sealed `pdf:<generation>` until remote settlement. Caller textual `operationId` is not used to decide remote/local reset disposition or Journal authority; P1-198 remains separate.

Focused deterministic production witnesses:
- `project_tools/test_p0_072_remote_save_reset_fence.js` — remote phase disposition, URL/site isolation, legacy ambiguity, same-id replacement, retry provenance, no-resurrection and P0-079 composition.
- `project_tools/test_p0_072_local_download_reset_fence.js` — prepared/admitted linearization, Chrome-start ordering, reset phase disposition, URL/site isolation, bound/unknown provenance, no-resurrection and P1-146/P0-039 composition.
- `project_tools/test_p1_146_download_start_settlement.js` remains the adjacent late-start-settlement witness and now also requires durable admission to occur before the Chrome side-effect call.

This is a **partial P0-072 implementation**, not closure. ReadLater→Upload / Delete→Trash destructive remote moves still lack the independent reset-surviving receipt required by canonical P0-072 evidence. P1-146 remains **ACTIVE** for exact automatic-download physical start/late-settlement/restart authority, and P1-198 remains **ACTIVE** for trusted worker-issued physical identity. Authorized real Chrome automatic-download reset/late-terminal evidence, real Yandex destructive-move evidence, and bounded/manual-resolution evidence remain required. P0-072 therefore remains **ACTIVE** and release readiness remains **NOT READY**.
