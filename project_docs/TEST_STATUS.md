...
S0-B now also has a passive production source-generation authority: `release_source_generation_v1.json` declares the single `public-suffix-js` relation and `project_tools/release_source_generation_authority.js` validates strict topology, exact candidate Git blobs, S0-A package-output membership, the exact `cpython-3.12.10-v1` runtime profile and isolated byte-for-byte regeneration. The permanent Repository Integrity workflow does not invoke this production verifier yet because generic project tooling remains intentionally pinned to CPython 3.12.14; profile-specific CI integration is a separate bounded step. See `RESEARCH_P1_231_S0B_PASSIVE_PRODUCTION_AUTHORITY_2026-09-21_EVIDENCE.md`.
