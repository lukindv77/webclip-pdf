...
P0-072 owns the rule that deleting/replacing Journal presentation is not evidence that an already admitted external effect was cancelled. The current production implementation now covers the known reset-sensitive classes: **Yandex remote save**, **automatic local download**, **ReadLater→Upload**, and **Delete→Trash**. This is still not P0-072 closure because restart/manual-resolution and authorized real Chrome/Yandex late/unknown-settlement evidence remain outstanding, and P1-090/P0-076/P1-198 retain separate authority boundaries.
...
These destructive receipts intentionally do **not** close P1-090: path/type/newly observed target metadata is not promoted into proof that an unknown destructive settlement affected the exact original object. They also do **not** close P0-076 generic per-entry revision + Journal-generation CAS; reset serialization and source id/createdAt/known-resource checks are bounded P0-072 defenses, not a replacement for the generic point-mutation owner. P1-198 remains the owner of trusted worker-issued live physical operation identity.
...
- `project_tools/test_p0_072_trash_move_reset_receipt.js` — detached Trash receipt, admitted/verified ordering, atomic guarded Journal delete, same-id replacement negative controls, and unchanged keep/local-only path.
- `project_tools/test_p1_146_download_start_settlement.js` remains the adjacent Chrome late-start-settlement witness.

P0-072 remains **ACTIVE**, not DONE. Production reset-survival is now implemented for the known external-effect classes, but closure still requires restart/crash/manual-resolution proof for detached destructive receipts, authorized real Chrome automatic-download reset/late-terminal evidence, authorized real Yandex destructive unknown/late-settlement evidence, and reconciliation with the separate P1-090/P0-076/P1-198 owners. Release readiness remains **NOT READY**.
