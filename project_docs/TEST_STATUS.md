...
P0-072 owns the rule that deleting/replacing Journal presentation is not evidence that an already admitted external effect was cancelled. The current bounded implementation covers **Yandex remote save**, **automatic local download**, and the **ReadLater→Upload destructive move receipt**. Delete→Trash remains the next separate destructive class.

For Yandex remote save, Journal clear-all, URL/site clear and staged import-replace no longer blindly clear/prune matching `pendingRemoteSaves`. In the same Journal IndexedDB transaction as the destructive reset, remote checkpoints are classified by external phase. `admitted-unknown` and `stale-unverified` receipts survive and are durably marked `supersededByJournalReset` with reset time/scope. A pre-upgrade `prepared` row that lacks the exact P0-079 PDF-cache receipt is conservatively treated as ambiguous and also survives. An exact synthetic/pre-admission `prepared` row remains reset-cancellable, while `remote-verified` is terminal remote truth and may be retired by reset.

For automatic local download, a newly created `pendingDownloads` intent is explicitly `downloadAdmissionPhase: prepared`. Immediately before `chrome.downloads.download(...)`, the worker re-reads that exact intent in a bounded readwrite transaction and advances it to `admitted-unknown`. If reset removed the still-prepared intent first, admission fails closed and Chrome is not called; if admission committed first, reset preserves the receipt. Matching admitted intents, bound DownloadItem rows and P0-039 `unknown` / `manual-resolution` rows survive reset with `supersededByJournalReset`. Existing P1-146 late-start settlement and P0-039 dead-letter semantics remain separate.

For ReadLater→Upload, Journal DB v8 adds a bounded independent `pendingDestructiveMoves` store. Each new read-move receives a worker-issued random receipt identity; caller textual `operationId` remains correlation metadata. Once source/target are known, a `prepared` detached receipt records source Journal id/createdAt, URL/site scope, source/target paths, known resource/public identity and account/root metadata. The legacy entry-co-located read-move checkpoint is then written only through a transaction that also re-reads this detached receipt and rejects missing/reset-superseded/source-mismatched authority.

Immediately before the actual `POST /resources/move`, the detached receipt is advanced to `admitted-unknown`. Journal clear-all, URL/site clear and staged import-replace now include the destructive-receipt store in the same reset transaction: `prepared` work may be dropped, while admitted/manual-resolution receipts survive and are marked `supersededByJournalReset`. If reset wins before admission, the move does not start. If admission wins first, later reset cannot erase reconciliation authority.

After the existing Yandex target verification succeeds, the receipt becomes `remote-verified` before local Journal finalization. Terminal Journal mutation and receipt consumption then occur in one transaction over `pendingDestructiveMoves`, Journal entries and meta. A missing/reset-superseded receipt or mismatched source Journal identity cannot mutate a replacement entry; a reset-superseded verified move completes as external history without resurrecting old Journal presentation. If an admitted move fails or has unknown settlement, the detached receipt remains durable with diagnostic error state; the old Journal checkpoint is updated only through the same detached-authority guard. Pre-admission failure removes the disposable receipt.

This ReadLater tranche intentionally does **not** claim P1-090 exact-object closure: target path/type/resource metadata still uses the existing verification logic, and unknown destructive settlement still requires the exact source-object proof owned by P1-090. It also does **not** claim full P0-076 generation/CAS closure; the detached receipt adds source id/createdAt/known-resource guards and reset serialization, while generic point-mutation revision authority remains P0-076.
...
- `project_tools/test_p0_072_local_download_reset_fence.js` — automatic local-download reset/admission composition.
- `project_tools/test_p0_072_read_move_reset_receipt.js` — Journal DB v8 detached read-move receipt, reset scope, prepared/admitted ordering, guarded checkpoint/finalize transactions, same-id replacement negative controls and explicit P1-090/P0-076 non-closure.
- `project_tools/test_p1_146_download_start_settlement.js` remains the adjacent Chrome late-start-settlement witness.

This is still a **partial P0-072 implementation**, not closure. Delete→Trash still lacks an independent reset-surviving receipt. Admitted ReadLater receipts after worker/browser interruption are durable but still require the bounded/manual-resolution and exact-object reconciliation path needed by P1-090; authorized real Yandex unknown/late-settlement evidence is also outstanding. P1-146, P1-090, P0-076 and P1-198 remain separate ACTIVE owners. P0-072 remains **ACTIVE** and release readiness remains **NOT READY**.
