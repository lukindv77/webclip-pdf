# Durable audit evidence — source URL data minimization / external persistence — 2026-08-30

Canonical current P-code owner/status authority remains exclusively in `AUDIT_REGISTRY.md`. This document preserves a completed **56-block** source-first data-confidentiality/integrity audit of the authoritative source URL as it crosses PDF rendering, retry/recovery persistence, Journal/export and Yandex upload/backup surfaces.
...
The audit does not prescribe one hash/key algorithm; it requires that plaintext secret-bearing URL components not be retained solely because equality/grouping needs an identifier.
...
- `AUDIT_REGISTRY.md` should remain unchanged in this docs-only tranche.
- P1-230 is deliberately not allocated.
- This session contains **56 completed audit blocks**.
