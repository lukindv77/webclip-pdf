...
| P1-009 | 60/60 | 48/48 | Journal filter browser PASS: 25 total / 20 first page / 2 title matches including record originally beyond page 1; AND/OR + comments/site/URL PASS; managed PDF 37,692 bytes. Later audit reopened scalability acceptance, so this remains semantics evidence. |
...
| P1-085 / P1-124 | 71/71 | 58/58 | Hung direct Journal read abort + bounded SW fallback PASS; same-worker `tabs.create` timeout/no-duplicate/late-receipt PASS. Later audit reopened P1-124 for MV3 restart durability. |
| P1-125 / P1-126 | 73/73 | 60/60 | `executeScript` settlement regression PASS; low-level worker `executeScript` exactly one call site. `tabs.get` bounded regression PASS; low-level worker `tabs.get` exactly one call site. Later audit reopened P1-125 same-URL document generation. |
